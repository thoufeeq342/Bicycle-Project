define([
    ...
    return function (placeOrderAction) {
        ...
        return wrapper.wrap(placeOrderAction, function (originalAction, paymentData, messageContainer) {
            ...
            var additionalPhoneNumber = $('[name="additional_phone_number"]').val(); // Fetch additional phone number

            if (deliveryDate || additionalPhoneNumber) {

                var payload = {
                    'cartId': quoteId,
                    'delivery_date': deliveryDate,
                    'additional_phone_number': additionalPhoneNumber, // Add to payload
                    'is_customer': isCustomer
                };

                if (!payload.delivery_date && !payload.additional_phone_number) {
                    return true;
                }
